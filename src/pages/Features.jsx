...
const CAPABILITIES = [
  ...
  {
    icon: Globe2,
    eyebrow: "Localized formats",
    title: "The exact format your bank expects",
    desc: "CMA Data for Indian banks, SBA for the USA, Innovate UK grants, PMEGP / Mudra / CGTMSE schemes — generated to the precise structure each institution requires.",
    points: ["India · USA · UK · UAE", "Govt scheme templates built-in", "Lender-ready, not generic"],
    visual: "formats",
  },
  {
    icon: BarChart3,
    eyebrow: "Investor-grade financials",
    title: "Every metric a lender or VC checks",
    desc: "DSCR, Current Ratio, IRR, Payback, MOIC and full P&L projections — calculated and rendered as interactive charts inside the report.",
    points: ["Live P&L & cash-flow charts", "Ratio analysis vs benchmarks", "5-year projections"],
    visual: "metrics",
  },
  {
    icon: FileDown,
...
const COMPARISON = [
  ...
  { label: "Investor metrics (IRR, MOIC)", rc: true, ca: "Sometimes", diy: false },
];

// ── Supported formats by region ──────────────────────────────────────────────
const REGIONS = [
  { flag: "🇮🇳", country: "India", items: ["CMA Data (SBI, PNB, BoB)", "PMEGP · Mudra · CGTMSE", "Stand-Up India"] },
  { flag: "🇺🇸", country: "United States", items: ["SBA 7(a) / 504", "Bank business plan", "Investor pitch pack"] },
  { flag: "🇬🇧", country: "United Kingdom", items: ["Innovate UK grants", "Start Up Loans", "Bank lending pack"] },
  { flag: "🌍", country: "Global / Investors", items: ["VC term-sheet model", "PE diligence pack", "Feasibility study"] },
];
...
const FAQ = [
  ...
  { q: "Will banks accept an AI-generated report?", a: "Absolutely. Reports follow the exact CMA / SBA / scheme format each institution expects, with all required ratios and projections. Thousands have been submitted and approved." },
  ...
  { q: "Which countries and schemes are supported?", a: "India (CMA, PMEGP, Mudra, CGTMSE), USA (SBA), UK (Innovate UK, Start Up Loans) and global investor formats — with more added regularly." },
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="relative overflow-hidden border-b">
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 py-20 sm:py-24 text-center">
          ...
          <p className="text-muted-foreground mt-5 max-w-2xl mx-auto text-lg leading-relaxed">
            Not just another report builder. Explore the AI interview, localized bank formats,
            investor-grade financials and one-click exports — in detail.
          </p>
          <div className="flex flex-wrap justify-center gap-4 mt-8 text-sm">
            {["AI interview", "30+ formats", "Live charts", "PDF · Word · Excel"].map((t) => (
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-6 py-20 sm:py-24">
        <div className="text-center mb-12 max-w-2xl mx-auto">
          ...
          <h2 className="text-3xl sm:text-4xl font-heading font-bold tracking-tight">
            Built for your region &amp; your scheme
          </h2>
          <p className="text-muted-foreground mt-3">
            One platform, every format banks, governments and investors ask for.
          </p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {REGIONS.map((r) => (
            <div key={r.country} className="rounded-3xl border bg-card p-6 hover:shadow-xl hover:-translate-y-0.5 transition-all">
              <div className="text-3xl mb-3">{r.flag}</div>
              <h3 className="font-semibold text-lg mb-3">{r.country}</h3>
              <ul className="space-y-2">
                {r.items.map((it) => (
                  <li key={it} className="flex items-start gap-2 text-sm text-muted-foreground">
                    <Check className="w-4 h-4 text-emerald-500 shrink-0 mt-0.5" /> {it}
                  </li>
                ))}
              </ul>
            </div>
    ...
    return (
      <Frame>
        <div className="grid grid-cols-2 gap-3">
          {["CMA Data", "SBA 7(a)", "PMEGP", "Innovate UK", "Mudra Loan", "VC Model"].map((f, i) => (
    ...
    return (
      <Frame>
        ...
        <div className="grid grid-cols-3 gap-2">
          {[{ k: "DSCR", v: "1.82" }, { k: "IRR", v: "24.6%" }, { k: "MOIC", v: "3.4x" }].map((m) => (
